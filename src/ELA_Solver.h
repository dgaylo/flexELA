 ...
 * When there are multiple ELA instances (set by \p nn in \ref ELA_Init()), one call to the following functions performs the operation on all instances
 * 
 * @file
 */
 ...
 * 
 * This routine is called at the start of an operator-split advection, when the internal vector source fraction is \f$\mathbf{s}^{(0)}\f$.
 * The vector dilation term \f$\tilde{\mathbf{c}}\f$ is calculated
 * \f[
 ...
 * 
 * This routine is called at the end of an operator-split advection when \f$ \tilde{\mathbf{c}} \f$ from ELA_SolverClearDilation() is no longer needed.
 * 
 */
 ...
 * \f]
 * 
 * @param u_div The velocity divergence, scaled by the timestep, \f$ \Delta t \frac{\partial u_d}{\partial x_d}\f$
 */
