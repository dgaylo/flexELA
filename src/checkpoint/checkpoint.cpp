...
// confirm correct array ordering
    ...
    if (isFortranBuild(header)) {
#endif
        throw std::invalid_argument("Array ordering mismatch in checkpoint file");
    }
    ...
    if (dom.n[0] != n_in[0] || dom.n[1] != n_in[1] || dom.n[2] != n_in[2]) {
        throw std::invalid_argument("Domain size mismatch in checkpoint file");
    }
    ...
    if (dom.nn != nn_in) {
        throw std::invalid_argument("ELA instance count mismatch in checkpoint file");
    }
    ...
    if (lCheckSum != lCheckSum_in) {
        throw std::invalid_argument(
            "Checksum (label) mismatch in checkpoint file: " + std::to_string(lCheckSum) + " vs " +
            std::to_string(lCheckSum_in)
        );
    }
    ...
    if (vCheckSum != vCheckSum_in) {
        throw std::invalid_argument(
            "Checksum (value) mismatch in checkpoint file: " + std::to_string(vCheckSum) + " vs " +
            std::to_string(vCheckSum_in)
        );
    }
}
...
void checkpoint::load(const char* filename, const domain::Domain& dom)
{
    ...
    default:
        ...
        throw std::invalid_argument(
            "Unknown checkpoint file version " + std::to_string(getVersionNumber(header)) + " in " +
            std::string(filename)
        );
    }
