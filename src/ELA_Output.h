 ...
 * \f]
 * for \f$ m \in 1\dots M^{n} \f$ and \f$ l \in 1\dots M^{n-1} \f$. Cells belonging to the blob \f$ m =0 \f$ are ignored.
 * Which blob \f$ m \f$ a cell \f$\Omega_{ijk}\f$ is in is determined by the label feild \p labels.
 * The number of rows in \f$\mathbf{Q}^{(n-1 \,\rightarrow\, n)}\f$, \f$ M^{n} \f$, is determined based on \p labels provided.
 * The number of columns, \f$ M^{n-1} \f$,  is not explicitly calculated.
 * The source fraction feild \f$ \mathbf{s}^{n-1}_{ijk} \f$ is the one currently stored in ELA isntance \p num.
 * 
