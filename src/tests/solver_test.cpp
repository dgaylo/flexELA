...
TEST(ELA,AdvectLabels) {
    ...
    for(int count=1; count<=100; count++) {
        ...
        for(auto n=0; n<ela::dom->nn; ++n) {
            ASSERT_NEAR(total_new[n]/total[n],1.0,1e-16*getFieldSize()) << "Failed on iteration " << count;
        }

        // advection should conserve each component of s
        for(auto n=0; n<ela::dom->nn; ++n) {
        for(auto l=0; l<=maxLabel; l++) {
            ASSERT_NEAR(individual_new[n][l]/individual[n][l],1.0,1e-16*getFieldSize()) << "Failed on iteration " << count << " for label " << l;
        }}
