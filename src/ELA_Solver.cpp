...
void advectRow(
    const fields::Helper<svec::SVector>& sRow, 
    const fields::Helper<const double>& fluxRow,
    ...
    )
{
    ...
    while(true) {
        ...
        const svec::SVector& s_upwind = (flux_loc > 0.0 ? 
            s_temp :    // F_{d+1/2}>0, s_{d+1} is upwind
